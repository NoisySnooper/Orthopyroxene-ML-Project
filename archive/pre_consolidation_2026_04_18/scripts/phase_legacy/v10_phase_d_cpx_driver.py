#!/usr/bin/env python3
"""v10 Phase D driver: 96 Optuna TPE studies for cpx pipeline.

Grid: 8 models x 2 targets x 2 tracks (cpx_only, cpx_liq) x 3 feature sets.

Mirrors scripts/v10_phase_c_driver.py but consumes cpx parquet + cpx
feature matrix builder.

Outputs:
  results/v10_optuna_studies/cpx/study_{model}_{target}_{track}_{fs}.joblib
  results/v10_optuna_best_params_cpx.json              (final)
  results/v10_optuna_best_params_cpx_partial.json      (incremental)
  logs/v10_phase_d_cpx_driver.log
...
TARGETS = ('T_C', 'P_kbar')
TRACKS = ('cpx_only', 'cpx_liq')
FEATURE_SETS = ('raw', 'alr', 'pwlr')

STUDIES_DIR = RESULTS / 'v10_optuna_studies' / 'cpx'
BEST_JSON = RESULTS / 'v10_optuna_best_params_cpx.json'
PARTIAL_JSON = RESULTS / 'v10_optuna_best_params_cpx_partial.json'
LOG_PATH = LOGS / 'v10_phase_d_cpx_driver.log'
...
def prepare_training(track, target, feature_set):
    ...
    X, feat_names = build_cpx_feature_matrix(df_tr, feature_set, use_liq=use_liq)
...
def main():
    ...
    try:
        ...
        log(f'Phase D cpx driver START  total={len(grid)} studies '
        ...
        log(f'Phase D cpx driver DONE  completed={len(all_results)}  '
