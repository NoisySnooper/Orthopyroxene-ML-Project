#!/usr/bin/env python3
"""v10 Phase E driver: 64 Optuna TPE studies for twopx pipeline.

Grid: 8 models x 2 targets x 1 track x 4 feature sets = 64 studies.

Feature sets: raw, alr, pwlr, twopx_components. Single track (twopx);
no opx_only / cpx_only variant since the two-pyroxene assumption
requires both minerals.

Outputs:
  results/v10_optuna_studies/twopx/study_{model}_{target}_{track}_{fs}.joblib
  results/v10_optuna_best_params_twopx.json
  results/v10_optuna_best_params_twopx_partial.json
  logs/v10_phase_e_twopx_driver.log
...
TARGETS = ('T_C', 'P_kbar')
TRACKS = ('twopx',)
FEATURE_SETS = ('raw', 'alr', 'pwlr', 'twopx_components')

STUDIES_DIR = RESULTS / 'v10_optuna_studies' / 'twopx'
BEST_JSON = RESULTS / 'v10_optuna_best_params_twopx.json'
PARTIAL_JSON = RESULTS / 'v10_optuna_best_params_twopx_partial.json'
LOG_PATH = LOGS / 'v10_phase_e_twopx_driver.log'
...
def prepare_training(track, target, feature_set):
    ...
    X, feat_names = build_twopx_feature_matrix(df_tr, feature_set)
...
def main():
    ...
    try:
        ...
        log(f'Phase E twopx driver START  total={len(grid)} studies '
        ...
        log(f'Phase E twopx driver DONE  completed={len(all_results)}  '
