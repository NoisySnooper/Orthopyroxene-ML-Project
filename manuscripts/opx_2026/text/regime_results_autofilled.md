...
*Generated: 2026-04-18 (Chunk B), revised with dual robustness check 2026-04-18 (Chunk C). Source tables: `tables/S8_5_{1,2,3,4}_regime_*.md` (S8.5.4 is the canonical robust audit). Source figures: `figures/fig24_per_regime_rmse_opx_liq.{pdf,png}`, `figures/fig25_per_regime_residual_violins_opx_liq.{pdf,png}`. Source audits: `results/opx_per_regime_claims_audit.csv` (axis-1 only, Chunk B) and `results/opx_per_regime_claims_audit_robust.csv` (two-axis, Chunk C). Pre-registration doc: `docs/preregistration/p_regime_preregistration.md` (registered 2026-04-17).*

## Framing

This section characterizes calibration-domain performance of the tuned ML baseline opx-liq thermobarometer across the pre-registered four-bin P partition `[0, 5, 15, 30, 100]` kbar (labels: shallow_crustal, deep_crustal_MASH, lithospheric_mantle, deeper_mantle). It is **not** a per-regime performance ranking. A claim of the form "method A outperforms method B in regime X" must pass a **dual robustness check**:

1. **Axis 1 — test-set sampling noise.** Bootstrap 95% CI on RMSE over paired (y_true, y_pred) rows (500 resamples, seed = SEED_BOOTSTRAP). the tuned ML baseline CI upper bound must lie below Putirka CI lower bound.
...
- **Exactly one bin meets the dual robustness check for a directional claim:** in the **shallow crustal** regime (0–5 kbar, n = 47), the tuned ML baseline (ElasticNet / raw) outperforms Putirka 29a for pressure with non-overlapping bootstrap CIs **and** zero seed variance (the ElasticNet cell is deterministic given fixed hyperparameters, so all 20 seeds produce RMSE = 2.66 kbar). tuned RMSE = 2.66 kbar [2.25, 3.13] vs Putirka 29a RMSE = 3.89 kbar [3.24, 4.47]. See Table S8.5.4 (robust) and Figure 24 right panel.
- **All other non-limited bins are "competitive with":** in the deep_crustal_MASH (n = 61) and lithospheric_mantle (n = 58) regimes, the tuned ML baseline and Putirka 95% CIs overlap for both T and P. Deep_crustal_MASH P_kbar uses MLP/raw, whose seed spread [2.01, 3.07] kbar also overlaps Putirka 29a's lower CI — the Chunk B point estimate of 2.08 was on the favorable tail of the seed distribution; the 20-seed mean is 2.54 kbar. This is transparency about training stochasticity, not a downgrade of the verdict.
...
| Regime                | n  | best tuned model cell    | tuned RMSE [95% CI]  | Seed RMSE mean [min, max] | Putirka best eq | Putirka RMSE [95% CI] | Robust verdict                         |
| --------------------- | -- | ---------------- | ------------------ | ------------------------- | --------------- | --------------------- | -------------------------------------- |
| shallow_crustal       | 47 | ElasticNet / raw | 2.66 [2.25, 3.13]  | 2.66 [2.66, 2.66] (det.)  | Putirka 29a     | 3.89 [3.24, 4.47]     | **the tuned ML baseline outperforms Putirka (robust)**   |
...
Figure 25 plots per-regime residual distributions (predicted minus observed) for the tuned ML baseline canonical base models on the opx_liq test split (T: ElasticNet/raw; P: MLP/raw). For both targets, the residual distribution narrows and centers on zero in the shallow_crustal and deep_crustal_MASH bins, consistent with the RMSE-based finding that the tuned ML baseline is most accurate in those regimes. Residual medians drift mildly away from zero in the lithospheric_mantle bin (underprediction of T, modest bias in P) and the deeper_mantle bin is too sparse to characterize.

## Manuscript-ready claim list

1. *"On the opx-liq test split, the tuned ML baseline thermobarometer outperforms Putirka (2008) for pressure in the shallow crustal regime (0–5 kbar, n = 47; the tuned ML baseline ElasticNet/raw RMSE 2.66 kbar, 95% CI [2.25, 3.13]; Putirka 29a RMSE 3.89 kbar, 95% CI [3.24, 4.47]; non-overlapping bootstrap CIs). The the tuned ML baseline cell is deterministic given fixed hyperparameters (20-seed RMSE spread of zero), so the claim is robust to model-fit stochasticity."* — **meets dual robustness check.**
2. *"Across the deep crustal / MASH and lithospheric mantle regimes, the tuned ML baseline and Putirka are competitive with one another (95% CIs overlap for both T and P) — we make no directional claim in these regimes. In deep_crustal_MASH pressure, the tuned ML baseline cell is an MLP and its 20-seed RMSE spread is [2.01, 3.07] kbar; the single-seed point estimate (2.08) is on the favorable tail of this spread, which is why we explicitly report 'competitive' rather than 'outperforms.'"* — **meets dual robustness check.**
3. *"The deeper mantle regime (> 30 kbar) is sample-size-limited in our test split (n = 8) and we report its results for transparency without a directional claim. The across-seed MLP/alr RMSE spread [3.78, 24.02] kbar further underscores the instability of this regime at n = 8."* — **meets dual robustness check.**

Any stronger phrasing elsewhere in the manuscript (e.g. "the tuned ML baseline beats Putirka in the deep crust") would fail the honesty bar and must be rewritten.
