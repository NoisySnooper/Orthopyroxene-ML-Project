...
This section characterizes calibration-domain performance of the the tuned ML baseline opx-liq thermobarometer across the pre-registered four-bin P partition `[0, 5, 15, 30, 100]` kbar (labels: shallow_crustal, deep_crustal_MASH, lithospheric_mantle, deeper_mantle). It is **not** a per-regime performance ranking. A claim of the form "method A outperforms method B in regime X" must pass a **two-axis honesty bar**:
...
- **Exactly one bin meets the two-axis honesty bar for a directional claim:** in the **shallow crustal** regime (0–5 kbar, n = 47), the tuned ML baseline (ElasticNet / raw) outperforms Putirka 29a for pressure with non-overlapping bootstrap CIs **and** zero seed variance (the ElasticNet cell is deterministic given fixed hyperparameters, so all 20 seeds produce RMSE = 2.66 kbar). tuned RMSE = 2.66 kbar [2.25, 3.13] vs Putirka 29a RMSE = 3.89 kbar [3.24, 4.47]. See Table S8.5.4 (robust) and Figure 24 right panel.
...
Figure 25 plots per-regime residual distributions (predicted minus observed) for the the tuned ML baseline canonical base models on the opx_liq test split (T: ElasticNet/raw; P: MLP/raw). For both targets, the residual distribution narrows and centers on zero in the shallow_crustal and deep_crustal_MASH bins, consistent with the RMSE-based finding that the tuned ML baseline is most accurate in those regimes. Residual medians drift mildly away from zero in the lithospheric_mantle bin (underprediction of T, modest bias in P) and the deeper_mantle bin is too sparse to characterize.

## Manuscript-ready claim list

1. *"On the opx-liq test split, the the tuned ML baseline thermobarometer outperforms Putirka (2008) for pressure in the shallow crustal regime (0–5 kbar, n = 47; the tuned ML baseline ElasticNet/raw RMSE 2.66 kbar, 95% CI [2.25, 3.13]; Putirka 29a RMSE 3.89 kbar, 95% CI [3.24, 4.47]; non-overlapping bootstrap CIs). The the tuned ML baseline cell is deterministic given fixed hyperparameters (20-seed RMSE spread of zero), so the claim is robust to model-fit stochasticity."* — **meets two-axis honesty bar.**
2. *"Across the deep crustal / MASH and lithospheric mantle regimes, the tuned ML baseline and Putirka are competitive with one another (95% CIs overlap for both T and P) — we make no directional claim in these regimes. In deep_crustal_MASH pressure, the the tuned ML baseline cell is an MLP and its 20-seed RMSE spread is [2.01, 3.07] kbar; the single-seed point estimate (2.08) is on the favorable tail of this spread, which is why we explicitly report 'competitive' rather than 'outperforms.'"* — **meets two-axis honesty bar.**
