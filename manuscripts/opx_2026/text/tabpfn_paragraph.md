...
- Regime-stratified TabPFN RMSE is spliced into `results/preregistered_scorecard_postcorrection.csv` as `tabpfn_rmse`, `tabpfn_rmse_lo`, `tabpfn_rmse_hi` columns, and the `winner` column's argmin now considers TabPFN as a 4th candidate alongside `v10_pre`, `v10_corrected`, and `external`.
